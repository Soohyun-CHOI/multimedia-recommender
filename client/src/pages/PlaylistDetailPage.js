...
function PlaylistDetailPage() {
  ...
  useEffect(() => {
    ...
    fetch(
      `http://${config.server_host}:${config.server_port}/playlist_max_mood/${playlistId}`
    )
      .then((res) => res.json())
      .then((resJson) => setMaxMood(resJson[0]));

    fetch(
      `http://${config.server_host}:${config.server_port}/collaborators/${playlistId}`
    )
      .then((res) => res.json())
      .then((resJson) => {
  ...
  const handleSubmit = async () => {
    for (let mediaId of deletedItems) {
      await fetch(
        `http://${config.server_host}:${config.server_port}/delete_media`,
        {
          method: "DELETE",
  ...
  const handleSubmitCollab = async () => {
    for (let userId of deletedCollab) {
      await fetch(
        `http://${config.server_host}:${config.server_port}/delete_collaborator`,
        {
          method: "DELETE",
          headers: { "Content-Type": "application/json" },
  ...
  const handleAddCollaborator = async () => {
    if (newCollabId) {
      const response = await fetch(
        `http://${config.server_host}:${config.server_port}/new_collaborator`,
        {
          method: "POST",
          headers: { "Content-Type": "application/json" },
          body: JSON.stringify({
      ...
      if (response.ok) {
        ...
        alert(
          "Failed to add collaborator. Please check the details and try again."
        );
      }
    } else {
    ...
    <>
      ...
      <div className="title-wrap">
        <div className="titles">
          <div className="title">
            {playlistContents[0] ? playlistContents[0].playlist_title : ""}
          </div>
          <div className="max-mood">
            {maxMood.max_mood && maxMood.max_mood.toUpperCase()}
          </div>
        </div>
      ...
      <div className="collab">
        <div>
          ...
          {editCollab.map((item) => (
            <div className="collab-item">
              ...
              {isEditingCollab && (
                <button onClick={() => handleDeleteCollab(item.user_id)}>
                  Delete
                </button>
              )}
            </div>
        ...
        <div className="collab-buttons">
          <div>
            <button onClick={toggleEditCollabMode} className="edit">
              {isEditingCollab ? "Cancel" : "Edit Collaborators"}
            </button>
            {isEditingCollab && (
              <button onClick={handleSubmitCollab} className="submit">
                Submit
              </button>
            )}
          </div>
          <div>
            {isAddingCollab ? (
              <>
                ...
                />
                <button onClick={handleAddCollaborator} className="submit">
                  Add
                </button>
              </>
            ) : (
              <button onClick={() => setIsAddingCollab(true)} className="add">
                Add Collaborators
              </button>
            )}
          </div>
