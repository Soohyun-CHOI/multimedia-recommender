...
function SuggestionPage() {
  ...
  useEffect(() => {
    fetch(
      `http://${config.server_host}:${config.server_port}/suggested_media?num_media=10`
    )
      .then((res) => res.json())
  ...
  return (
    <>
      ...
      <div className="media-controls">
        ...
        <button onClick={() => fetchAdditionalMedia("bk")}>
          Get More Books
        </button>
        <button onClick={() => fetchAdditionalMedia("tv")}>
          Get More Shows
        </button>
        <button onClick={() => fetchAdditionalMedia("mv")}>
          Get More Movies
        </button>
        <button onClick={() => fetchAdditionalMedia("mu")}>
          Get More Music
        </button>
        <button onClick={() => fetchAdditionalMedia("gm")}>
          Get More Games
        </button>
      </div>
          ...
          {resultData.map((suggested_media) => (
            <div className="media" key={suggested_media.media_id}>
              <div className="media-type">
                {(suggested_media.media_type || " ").toUpperCase()}
              </div>
              <NavLink to={`/media/${suggested_media.media_id}`}>
                {suggested_media.media_type === "mv" && (
                  <img
                    onError={(e) => {
                      e.target.onerror = null;
                      e.target.src = movieDefaultImage;
                    }}
                    src={suggested_media.image || movieDefaultImage}
                    alt="media image"
                  />
                )}
                {suggested_media.media_type === "gm" && (
                  <img
                    onError={(e) => {
                      e.target.onerror = null;
                      e.target.src = gameDefaultImage;
                    }}
                    src={suggested_media.image || gameDefaultImage}
                    alt="media image"
                  />
                )}
                {suggested_media.media_type === "mu" && (
                  <img
                    onError={(e) => {
                      e.target.onerror = null;
                      e.target.src = musicDefaultImage;
                    }}
                    src={suggested_media.image || musicDefaultImage}
                    alt="media image"
                  />
                )}
                {suggested_media.media_type === "bk" && (
                  <img
                    onError={(e) => {
                      e.target.onerror = null;
                      e.target.src = bookDefaultImage;
                    }}
                    src={suggested_media.image || bookDefaultImage}
                    alt="media image"
                  />
                )}
                {suggested_media.media_type === "tv" && (
                  <img
                    onError={(e) => {
                      e.target.onerror = null;
                      e.target.src = TVDefaultImage;
                    }}
                    src={suggested_media.image || TVDefaultImage}
                    alt="media image"
                  />
                )}
              </NavLink>
              <NavLink
                to={`/media/${suggested_media.media_id}`}
              ...
              {!addedMedia[suggested_media.media_id] && (
                <button
                  onClick={() => handleAddMedia(suggested_media.media_id)}
                >
                  ADD
                </button>
              )}
